...
| State | Process memory | Upstash Redis REST with a 24-hour TTL |
...
## Why persistence changes on Vercel

A Vercel Function is stateless: two consecutive requests can run in different Python processes.
SignalPrep therefore writes the validated `InterviewSession` and original RAG source texts to
Upstash. When a new function instance receives an answer, it reloads the session and rebuilds the
small TF-IDF index. Both records expire according to `SESSION_TTL_SECONDS`.
...
3. Add an Upstash Redis integration to the Vercel project.
...
7. Confirm that the same session still loads after a fresh function instance handles the request.
